...
******************************************************************************/
FORCE_INLINE bool kff_write_handler(u32 control, u32 addr, u32 data)
{
    if (!(control & C64_IO1))
    {
        switch (addr & 0xff)
...
{
    kff_init();
    reu_handler_init(kff_reu_handler);
}

// REU support and allow SDIO and USB to be used while handling C64 bus access
C64_REU_BUS_HANDLER(kff)

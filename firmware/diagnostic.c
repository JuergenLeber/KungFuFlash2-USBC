...
static bool diag_send_result(bool saving)
{
    ...
    sprint(scratch_buf, "%d   ", cfg_file.phi2_offset);
    usb_send_text(scratch_buf);
    c64_send_text(cfg_file.phi2_offset ? COLOR_YELLOW : COLOR_LIGHTGREEN,
                  16, 7, scratch_buf);
    ...
    c64_interface(false);
    if (c64_is_reset()) // C64 is reset if new button press is detected
    {
        return false;
    }
    else if (!diag_wait_timeout)
    {
        c64_reset();
    }
...
static void diag_handle_button(void)
{
    ...
    if (!long_press)
    {
        ...
        c64_reset();    // Reset C64 after phi2 adjustment
    }
    else
    {
        if (button & MENU_BTN)
        {
            cfg_file.phi2_offset = 0;
            c64_reset();    // Reset C64 after phi2 adjustment
        }

        if (button & SPECIAL_BTN)
        {
            diag_save_and_restart(cfg_file.phi2_offset);
        }
    }

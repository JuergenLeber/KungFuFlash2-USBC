    ...
    while (true)
    {
        ...
        switch (reply)
        {
            ...
            case REPLY_BASIC:
                ...
                should_save_cfg = persist_basic_selection();
                cmd = CMD_WAIT_SYNC;
                break;

            case REPLY_KILL:
                cfg_file.boot_type = CFG_KILL;
                should_save_cfg = persist_basic_selection();
                cmd = CMD_WAIT_SYNC;
                break;

            case REPLY_BASIC_C128:
                cfg_file.boot_type = CFG_BASIC_C128;
                should_save_cfg = persist_basic_selection();
                cmd = CMD_WAIT_SYNC;
                break;
...
static bool format_path(char *buf, bool include_file)
{
    ...
    *buf++ = ' ';
    ...
    u16 len;
    for (len = 0; len < sizeof(cfg_file.path) && cfg_file.path[len]; len++)
    {
        buf[len] = cfg_file.path[len];
    }
    ...
    else if (len > DIR_NAME_LENGTH-1)
    {
        index = len - (DIR_NAME_LENGTH-1);
        buf[index] = '.';
        buf[index+1] = '.';
    }
    else if (len == 1 && buf[0] == '/')
